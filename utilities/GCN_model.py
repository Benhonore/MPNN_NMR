...
from .modules.gcn_train import do_train
...
class GCNmodel():
	...
	def init_model(self):
		self.check_params()
		self.model = GCN(10, self.params['embedding_size'], self.params['dropout'])
		...
		with torch.no_grad():
			for batch in test_loader:
				...
				for i in (list(batch.y[:,0].detach().numpy())):
					true.append(float(i))
				pred = self.model(self.params['num_layers'], batch.x.t()[:10].t(), batch.edge_index)
